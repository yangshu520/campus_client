export default class request {
	constructor(options) {
		...
		this.config = {
			...
			load: options.load === false ? false : true, // false 为打开
			isFactory: options.isFactory === false ? false : true,
			loadMore: options.loadMore === false ? false : true,
			loadTitip: options.loadTitip?options.loadTitip:"正在加载中",
			timeOut:options.timeOut?options.timeOut:10000,
			failTip:options.failTip?options.failTip:"请求超时,请重试"
		};
	}
		...
		return new Promise((resolve, reject) => {
			...
			let requestData = {
				...
				fail: (err) => {
					...
					reject(err);
					if(this.config.failTip){
						uni.showToast({
							title: this.config.failTip, //提示后台接口抛出的错误信息
							icon: "none",
							duration: 3000
						});
					}else{
						console.log("请求失败,接口为: "+ requestInfo.url)
					}
