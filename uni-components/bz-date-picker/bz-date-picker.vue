      ...
      <view class="u-picker-body">
        ...
        >
          <picker-view-column v-if="!reset">
            <view v-for="(item, index) in dates" :key="index" class="u-column-item">
              {{ item.label }}
            </view>
          </picker-view-column>
          <picker-view-column v-if="!reset">
 ...
 * @example
 */
import { dateFormat } from '@/common/utils/public.js'
  ...
  methods: {
    ...
    setDates() {
      this.dates = this.generateArray(1, this.days).map(i => {
        return this.getDate(+new Date(this.showTime) + (i - 1) * 24 * 60 * 60 * 1000, i - 1)
      })
    ...
    verifyValue(value) {
      ...
      let curMinute = this.formatNumber(
        Math.ceil(dateFormat('mm', this.showTime) / this.minuteStep) * this.minuteStep
      )
